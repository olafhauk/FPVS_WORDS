...
from mne import read_vectorview_selection as read_selection  # depends on version on MNE-Python
...
def get_MEG_ROI_channel_names(meg_selections, info):
    """Get channel names for MEG ROIs, mags and grads seperately.
    Parameters:
    meg_selections: list of str
        List of available channel selections (such as'Left-occipital').
    info: instance of Info
        The information of the sensor configuration. 

    Returns:
    channel_ROIs: dict
        Lists of channel names for magnetometers and gradiometers separately.
    """
    channel_ROIs = {}
    for roi in meg_selections:
        channel_ROIs['Mag ' + roi] = []
        channel_ROIs['Grad ' + roi] = []
        ch_names = read_selection(name=roi, info=info)
        for cn in ch_names:
            if cn[-1] == '1':
                channel_ROIs['Mag ' + roi].append(cn)
            else:
                channel_ROIs['Grad ' + roi].append(cn)
    return channel_ROIs
...
def plot_evo_topomap(evoked, times, chtypes, fname_fig):
    ...
    for chtype in chtypes:
        ...
        fig = evoked.plot_topomap(times=times, ch_type=chtype, vlim=(0., None),
                                  time_format='',
...
def psds_across_harmonics(psds, freqs, basefreq, oddfreq, n_harms, n_bins,
                          n_gap=0, skip_harm=0, method='sum'):
    """Combine across harmonics of oddball frequency, without base frequency.

    Parameters:
        psds: instance or list of Evoked or Source Estimate
        ...
        skip_harm: int
            Number of lower harmonics to skip during summing (default: 0).
        method: str
            'sum' or 'avg'
    ...
    Returns:
        psd_harms: instance of Evoked or Source Estimate,
            data of dimension (2 * (n_bins + n_gaps) + 1).
        ...
        psd_harms_epos: list of Evoked or Source Estimate
            List of epochs (like psd_harms) for individual harmonics.
    ...
    psd_harms, topo, topos, freqs_harm, psd_harms_epos = [], [], [], [], []
    ...
    for psd in psds:
        ...
        if basefreq is not None:
            ...
            # basefreq
            freqs_harm_tmp = _get_valid_harmonics(freqs, basefreq, oddfreq, n_harms)

        else:

            freqs_harm_tmp = np.arange(oddfreq, (n_harms + 1) * oddfreq, oddfreq)

        # skip some lower harmonics if specified
        if skip_harm > 0:
            print('Skipping first %d harmonics in summing.' % skip_harm)
            freqs_harm_tmp = freqs_harm_tmp[skip_harm:]

        # find indices corresponding to valid harmonic frequencies
        harm_idx = [np.abs(ff - freqs).argmin() for ff in freqs_harm_tmp]

        print('Frequencies:\n')
        [print(i, freqs[ii], freqs_harm_tmp[i]) for (i, ii) in enumerate(harm_idx)]
        ...
        topos_mat = np.zeros([data.shape[0], len(freqs_harm_tmp)])

        # collect epochs around harmonics
        psd_harms_epos_tmp = []
        ...
        for (ii, iii) in enumerate(harm_idx):
            ...
            # add epoch for this harmonic
            psd_harms_epos_tmp.append(data_now)

        # average if requested
        if method == 'avg':

            data_harms = data_harms / freqs_harm_tmp.size
        ...
        if type(psd) is SourceEstimate:
            ...
            psd_harms_tmp = SourceEstimate(data=data_harms, vertices=vertices,
                                           tmin=tmin, tstep=tstep)

            # topography for combined responses at centre frequency
            t = data_harms[:, n_bins + n_gap]

            topo_tmp = SourceEstimate(
                data=t[np.newaxis, :].T, vertices=vertices, tmin=0.,
                tstep=0.001)

            # topographies for all harmonics
            topos_tmp = SourceEstimate(
                data=topos_mat, vertices=vertices, tmin=0., tstep=0.001)

            for [ee, epo] in enumerate(psd_harms_epos_tmp):
                psd_harms_epos_tmp[ee] = \
                    SourceEstimate(
                        data=epo, vertices=vertices, tmin=0., tstep=0.001)

        elif type(psd) is EvokedArray:

            info_psd = psd.info
            # in order to turn frequencies into latencies
            sfreq = 1. / freq_resol  # to display samples as time points

            # ch_types = 102 * ['grad', 'grad', 'mag'] + 64 * ['eeg']
            # info_evo = create_info(ch_names=info.ch_names, sfreq=sfreq, ch_types=ch_types)

            nave = psd.nave

            psd_harms_tmp = EvokedArray(data_harms, info_psd, tmin=tmin, nave=nave)
            ...
            topo_tmp = EvokedArray(
                t[np.newaxis, :].T, info_psd, tmin=0., nave=nave)

            # topographies for all harmonics
            topos_tmp = EvokedArray(topos_mat, info_psd, tmin=0.)

            for [ee, epo] in enumerate(psd_harms_epos_tmp):
                psd_harms_epos_tmp[ee] = \
                    EvokedArray(epo, info_psd, tmin=tmin, nave=nave)

        else:

            print('Type of ''psd'' not known (%s)' % type(psd))

        psd_harms.append(psd_harms_tmp)
        topo.append(topo_tmp)
        topos.append(topos_tmp)
        freqs_harm.append(freqs_harm_tmp)
        psd_harms_epos.append(psd_harms_epos_tmp)

    # for compatibility with previous versions
    if len(psds) == 1:
        psd_harms = psd_harms[0]
        topo = topo[0]
        topos = topos[0]
        freqs_harm = freqs_harm[0]
        psd_harms_epos = psd_harms_epos[0]

    return psd_harms, topo, topos, freqs_harm, psd_harms_epos
...
def _get_valid_harmonics(freqs, basefreq, oddfreq, n_harms):
    ...
    for idx in np.flip(del_idx):
        ...
        # delete one harmonic of oddball that coincides with base frequency
    ...
    for ff in np.arange(0, data.shape[1]):  # for frequencies
        ...
        for cc in np.arange(0, data.shape[0]):  # for channels or vertices
            ...
            if minmax:
                ...
                baseline = np.delete(baseline, (minidx, maxidx))
...
def plot_psd_as_evo(psd, sbj_path, picks=None, txt_label='',
    ...
    Parameters:
        ...
        sbj_path: path where figures will be saved.
        ...
        ylim: dict
            y axis limits for channels types ('grad', 'mag', 'eeg'). Scale to
            extrema if None.
    Returns:
        figs: list
    ...
    for (cc, ff) in zip(ch_types, figs):
        ...
        for fig_format in ['.jpg']:  # jpg doesn't work, png does
            ...
            fname_fig = op.join(sbj_path, txt_label + '_' +
                                str(cc) + fig_format)
...
def remove_subjects_extras(sbj_list):
    '''Remove subjects that don't have extra condition from list.
    Parameters
    ----------
    sbj_list: list of int
      List of subject IDs.
    Returns
    -------
    sbj_list_new: list of int
        List of subject IDs with non-extras removed.
    '''
    sbj_list_new = sbj_list.copy()
    # remove subjects from missing extras
    for mm in config.missing_extras:
        if mm in sbj_list_new:
            del sbj_list_new[sbj_list_new.index(mm)]
